        ...
        if swan_type == "ragnarok":  # Gods fall: Core powers lose status
            for name, agent in self.world.agents.items():
                if agent.tier == "Core Power":
                    ...
                    agent.morale = -0.6
                    agent.add_memory(
                        "Global economic collapse hits core powers. We are falling.",
                        importance=10,
                    )
            self.world.feed.publish(
                "WORLD_SYSTEM",
                "The old football order is collapsing. Core powers in crisis.",
            )
            ...
            if target in self.world.agents:
                ...
                agent.add_memory(
                    "A legendary heritage has been unlocked. We are now the masters of the game.",
                    importance=10,
                )
                self.world.feed.publish(
                    "WORLD_SYSTEM",
                    "China PR has undergone a status ascension. The East is rising.",
                )
                # Symbolic capital transfer from one former core power to the new ascendant.
                core_pool = [
                    n
                    for n, a in self.world.agents.items()
                    if n != target and a.tier == "Core Power"
                ]
                if core_pool:
                    rng = named_py_rng(
            ...
            self.world.feed.publish(
                "WORLD_SYSTEM",
                "All matches suspended. The football world turns into a pure social battleground.",
            )
    ...
    def get_gossip_keywords(self):
        ...
        for agent in self.world.agents.values():
            for memory in agent.memory_stream[-5:]:
                words.extend(
                    memory["content"].lower().replace(".", "").replace(",", "").split()
                )

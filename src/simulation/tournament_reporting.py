...
def _ordered_context_fields(*groups, extra=()):
    ordered = tuple(dict.fromkeys(name for group in groups for name in group))
    return ordered + tuple(name for name in extra if name not in ordered)


def _require_stage_context(context, expected, stage):
    actual = set(context)
    ...
    if missing or unexpected:
        ...
        raise TypeError(f"{stage} context mismatch: {'; '.join(details)}")


def _select_context(context, fields, **updates):
    selected = {name: context[name] for name in fields}
...
_MATCH_REPORT_FIELDS = _ordered_context_fields(
    _RESULT_PAYLOAD_FIELDS,
    _XG_CONTEXT_FIELDS,
    _MATCH_DEBUG_FIELDS,
    _REPORT_REPLAY_FIELDS,
...
_NARRATIVE_SOCIAL_FIELDS = _ordered_context_fields(
    _FACTS_LEDGER_FIELDS,
    _NARRATIVE_ATMOSPHERE_FIELDS,
    _MEDIA_MATRIX_FIELDS,
    _POST_MATCH_DIALOGUE_FIELDS,
...
class TournamentReportingMixin:
    def _build_result_payload(
        ...
        *,
        ...
        drama_score = min(
            ...
            0.25
            + abs(s1 - s2) * 0.12
            + (0.2 if s1 == s2 else 0.0)
            + 0.15 * referee["strictness"],
        )
        ...
        if is_knockout and pen1 is not None and winner_name:
            print(f"  [PENALTIES] {winner_name} wins {pen1}-{pen2} on penalties.")
        ...
        if micro_replay_mode or (aff_on and micro_summary is None):
            try:
                if micro_replay_mode:
                    ...
                    from src.match_engine.match_affective_runner import (
    ...
    def _report_match_result(self, **context):
        _require_stage_context(context, _MATCH_REPORT_FIELDS, "match report")
        drama_score, key_event, verdict_json = self._build_result_payload(
            **_select_context(context, _RESULT_PAYLOAD_FIELDS)
        )
        xg_context_line = self._build_xg_context(
            **_select_context(context, _XG_CONTEXT_FIELDS)
        )
        self._audit_match_debug(
            **_select_context(
                context,
                _MATCH_DEBUG_FIELDS,
                xg_context_line=xg_context_line,
            )
        )
    ...
    def _build_facts_ledger(
        ...
        *,
        ...
        facts_ledger = {
            ...
            "xg": f"{xg1}-{xg2}",
            "stage": stage_name,
            "t1_pressing_intensity": round(
                float(a1.tactical_controls.get("pressing_intensity", 0.5)), 2
            ),
            "t1_risk_budget": round(
                float(a1.tactical_controls.get("risk_budget", 0.5)), 2
            ),
            "t1_line_height": round(
                float(a1.tactical_controls.get("line_height", 0.5)), 2
            ),
            "t1_rotation_aggressiveness": round(
                float(a1.tactical_controls.get("rotation_aggressiveness", 0.5)), 2
            ),
            "t2_pressing_intensity": round(
                float(a2.tactical_controls.get("pressing_intensity", 0.5)), 2
            ),
            "t2_risk_budget": round(
                float(a2.tactical_controls.get("risk_budget", 0.5)), 2
            ),
            "t2_line_height": round(
                float(a2.tactical_controls.get("line_height", 0.5)), 2
            ),
            "t2_rotation_aggressiveness": round(
                float(a2.tactical_controls.get("rotation_aggressiveness", 0.5)), 2
            ),
            "drama_score": round(float(drama_score), 2),
            "effective_status": f"{eff_status_1:.1f}-{eff_status_2:.1f}",
            "t1_tactical_preset": getattr(
                getattr(a1, "coach_profile", None), "preferred_preset", ""
            ),
            "t2_tactical_preset": getattr(
                getattr(a2, "coach_profile", None), "preferred_preset", ""
            ),
            "t1_through_ball_bias": round(
                float(getattr(a1, "tactical_vector", {}).get("through_ball_bias", 0.5)),
                2,
            ),
            "t2_through_ball_bias": round(
                float(getattr(a2, "tactical_vector", {}).get("through_ball_bias", 0.5)),
                2,
            ),
        }
        if xg_context_line:
            facts_ledger["xg_alignment_note"] = xg_context_line
        if micro_summary is not None and getattr(
            micro_summary, "cognitive_plans", None
        ):
        ...
        facts_ledger["regulation_score"] = (
            f"{reg_s1}-{reg_s2}" if went_to_extra_time else f"{s1}-{s2}"
        )
        ...
        facts_ledger["penalty_score"] = (
            f"{pen1}-{pen2}" if pen1 is not None else "not_played"
        )
        tg = int(s1 + s2)
    ...
    def _publish_media_matrix(
        ...
        *,
        ...
        try:
            ...
            if social_post:
                self.world.feed.publish(
                    "SOCIAL_TREND", social_post, tags=["social", "chaos", stage_name]
                )
        ...
        dialogue_pack = self.dialogue_engine.run_post_match_dialogue(
            ...
            context={
                ...
                "cognitive_plans": (
                    ...
                    and getattr(micro_summary, "cognitive_plans", None)
                    else []
                ),
            },
    ...
    def _run_narrative_and_social(self, **context):
        _require_stage_context(
            context,
            _NARRATIVE_SOCIAL_FIELDS,
            "narrative and social",
        )
        facts_ledger = self._build_facts_ledger(
            **_select_context(context, _FACTS_LEDGER_FIELDS)
        )
        verdict_json, atmosphere_chaos = self._apply_narrative_atmosphere(
            **_select_context(context, _NARRATIVE_ATMOSPHERE_FIELDS)
        )
        if not context["is_knockout"]:

...
class WorldEngine:
    def __init__(
        self, stats_df, tactical_map=None, initialization_rng=None,
        root_seed=42,
    ):
        self.agents = {}
        ...
        for team_name, row in stats_df.iterrows():
            ...
            agent_seed = derive_seed(
                self.root_seed, "society_agent", team_name,
            )
    ...
    def run_day(self):
        ...
        active_rng = named_py_rng(
            self.root_seed, "world_day", day_identity, "active_agents",
        )
        
        # 1. Action Phase
        active_this_day = active_rng.sample(
            list(self.agents.keys()), int(len(self.agents) * 0.15),
        )
        
        for name in active_this_day:
            agent = self.agents[name]
            agent_rng = named_py_rng(
                self.root_seed, "world_day", day_identity,
                "agent_action", name,
            )
        ...
        if self.current_date.weekday() in [5, 6]:
            self.simulate_big_matches(rng=named_py_rng(
                self.root_seed, "world_day", day_identity, "headline_match",
            ))

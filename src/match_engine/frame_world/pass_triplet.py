...
class PassTripletTransition(nn.Module):
    ...
    def forward(self, features):
        raw = torch.tanh(self.net(features)).reshape(-1, 3, 2)
        caps = raw.new_tensor(
            [self.cfg.ball_cap, self.cfg.player_cap, self.cfg.player_cap]
        )[None, :, None]
        return raw * caps

    def predict(self, current, velocities, features, enabled=True):
        baseline = current + velocities * (
            self.cfg.horizon_steps / self.cfg.sample_rate_hz
        )
...
def load_pass_triplet(path):
    ...
    cfg = PassTripletConfig(**payload["config"])
    model = PassTripletTransition(cfg)

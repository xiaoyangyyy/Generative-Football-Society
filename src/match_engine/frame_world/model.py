...
class GraphTemporalFrameWorldModel(nn.Module):
    def __init__(self, cfg: FrameWorldConfig = FrameWorldConfig()):
        ...
        self.logvar_head = nn.Sequential(
            nn.Linear(d * 2, d), nn.SiLU(), nn.Linear(d, 2)
        )
    ...
    def forward(self, positions, velocities, visible, teams):
        ...
        features = torch.cat(
            [positions, velocities, visible[..., None].float(), team_onehot], dim=-1
        )
        tokens = (
            self.input(features)
            .permute(0, 2, 1, 3)
        ...
        for index in range(len(self.cfg.horizons)):
            h = self.horizon.weight[index][None, None].expand(batch, entities, -1)
            fused = torch.cat([encoded, h], dim=-1)
            outputs.append(0.20 * torch.tanh(self.mean_head(fused)))
            logvars.append(self.logvar_head(fused).clamp(-9, 1))
        pooled = (encoded * visible[:, -1, :, None]).sum(1) / visible[:, -1].sum(
            1, keepdim=True
        ).clamp_min(1)
        calibrated = (
            torch.stack(logvars, 1) + self.variance_log_offset[None, :, None, None]
        )
...
def load_frame_world(path):
    ...
    cfg = FrameWorldConfig(**payload["config"])
    model = GraphTemporalFrameWorldModel(cfg)

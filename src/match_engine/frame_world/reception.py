...
class ReceptionChainModel(nn.Module):
    def __init__(self, cfg=ReceptionConfig()):
        ...
        self.completion = nn.Linear(d, 1)
        self.next_action = nn.Linear(d, len(NEXT_ACTIONS))

    def forward(self, features):
        state = self.encoder(features)
        return self.completion(state).squeeze(-1), self.next_action(state)
...
def apply_reception_state(complete, receiver, defender, teams, next_action):
    index = receiver if complete else defender
    return ReceptionState(
        "complete" if complete else "turnover",
        int(index),
        int(teams[index]) if 0 <= index < len(teams) else -1,
        next_action,
    )
...
def load_reception_chain(path):
    ...
    cfg = ReceptionConfig(**payload["config"])
    model = ReceptionChainModel(cfg)

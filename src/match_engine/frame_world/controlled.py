...
class ActionConditionedFrameWorld(nn.Module):
    def __init__(self, cfg: ControlledFrameConfig = ControlledFrameConfig()):
        ...
        nn.init.constant_(self.logvar[-1].bias, -7)
    ...
    def forward(self, positions, velocities, visible, teams, actions):
        ...
        encoded = self.graph(
            state + action_tokens, src_key_padding_mask=~visible[:, -1].bool()
        )
        fused = torch.cat([encoded, action_tokens], -1)
        return 0.003 * torch.tanh(self.delta(fused)), self.logvar(fused).clamp(-10, 0)
...
def observed_actions(
    velocities: torch.Tensor, visible: torch.Tensor, cfg: ControlledFrameConfig
) -> torch.Tensor:
    acceleration = (velocities[:, -1] - velocities[:, -2]) * cfg.sample_rate_hz
    valid = (visible[:, -1] & visible[:, -2])[..., None]
    acceleration = torch.where(
        valid, acceleration, torch.zeros_like(acceleration)
    ).clamp(-cfg.max_acceleration, cfg.max_acceleration)
...
def transition(model, positions, velocities, visible, teams, actions):
    ...
    acceleration = actions[..., :2].clamp(
        -model.cfg.max_acceleration, model.cfg.max_acceleration
    ) * actions[..., 2:3].clamp(0, 1)
    ...
    next_velocity = (velocities[:, -1] + acceleration * dt + residual / dt).clamp(
        -model.cfg.max_velocity, model.cfg.max_velocity
    )
    return next_position.clamp(-0.15, 1.15), next_velocity, logvar
...
def rollout(model, positions, velocities, visible, teams, steps, policy=None):
    ...
    vis = visible.clone()
    predictions = []
    initial = observed_actions(vel, vis, model.cfg)
    for step in range(steps):
        actions = (
            initial * (0.65**step)
            if policy is None
            else policy(step, pos, vel, vis, teams)
        )
        next_pos, next_vel, _ = transition(model, pos, vel, vis, teams, actions)
        predictions.append(next_pos)
        pos = torch.cat([pos[:, 1:], next_pos[:, None]], 1)
        vel = torch.cat([vel[:, 1:], next_vel[:, None]], 1)
        vis = torch.cat(
            [
                vis[:, 1:],
                vis[
...
def load_controlled_frame_world(path):
    ...
    cfg = ControlledFrameConfig(**payload["config"])
    model = ActionConditionedFrameWorld(cfg)

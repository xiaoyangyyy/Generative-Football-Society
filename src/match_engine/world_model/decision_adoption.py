...
def register_coach_action_decision(
    ...
    *,
    ...
    feedback_context = dict(opponent_information_feedback_context or {})
    if feedback_context:
    ...
    if belief_space_meta_context:
        ...
        if not belief_space_meta_plan_audit_is_valid(
            ...
            belief_space_meta_context = {}
    active_probe_context = dict(llm_active_probe_context or {})
    if active_probe_context:
        from src.match_engine.world_model.active_probe import (
            active_probe_audit_is_valid,
        )

        probe = active_probe_context.get("probe") or {}
        if (
            not active_probe_audit_is_valid(active_probe_context)
            or str(active_probe_context.get("team_id")) != str(team_id)
            or str(active_probe_context.get("checkpoint_signature"))
            != str(checkpoint_signature)
            or str(active_probe_context.get("environment_signature"))
            != str(environment_signature)
    ...
    if mechanism_stress_context:
        ...
        stress_test = mechanism_stress_context.get("stress_test") or {}
        if (
            not mechanism_stress_test_audit_is_valid(
                mechanism_stress_context
            )
            or str(mechanism_stress_context.get("team_id")) != str(team_id)
            or str(mechanism_stress_context.get("checkpoint_signature"))
            != str(checkpoint_signature)
            or str(mechanism_stress_context.get("environment_signature"))
            != str(environment_signature)
            or str(stress_test.get("action")) != selected
        ):
            mechanism_stress_context = {}
    ...
    if adaptation_context:
        ...
        if not opponent_information_adaptation_audit_is_valid(
            ...
            adaptation_context = {}
    decision_brief_context = dict(llm_decision_brief_context or {})
    if decision_brief_context:
    ...
    record = {
        ...
        "llm_risk_certificate_context": dict(
            llm_risk_certificate_context or {}
        ),
        "llm_distributional_claim_context": dict(
            llm_distributional_claim_context or {}
        ),

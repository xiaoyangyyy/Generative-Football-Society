...
const renderActionAdoptionWithoutM2ResearchControl=renderActionAdoption;
renderActionAdoption=studio=>{renderActionAdoptionWithoutM2ResearchControl(studio);renderM2ResearchControl(studio?.evidence?.outcome_aligned_m2_study)};
...
const renderManagerIntelligenceWithoutOfficialActionExecution=renderManagerIntelligence;
renderManagerIntelligence=(command,configured)=>{renderManagerIntelligenceWithoutOfficialActionExecution(command,configured);const debrief=command?.postmatch_debrief,evidence=debrief?.world_model_action_execution;if(!configured||!debrief?.available||!evidence)return;if(evidence.available){appendOfficialActionExecution(matchdayAttribution,evidence,'正式比赛中的世界模型动作采用')}else{const unavailable=document.createElement('p');unavailable.className='status';unavailable.textContent='世界模型动作执行证据：'+(evidence.reason||'不可用');matchdayAttribution.append(unavailable)}};
const renderManagerIntelligenceWithoutWorldEvolutionThread=renderManagerIntelligence;
...
const appendManagerWorldEvolutionThreadWithoutReviewedFutures=appendManagerWorldEvolutionThread;
appendManagerWorldEvolutionThread=(host,thread)=>{appendManagerWorldEvolutionThreadWithoutReviewedFutures(host,thread);const stage=(thread?.stages||[]).find(row=>row.stage_id==='prematch_future_review'),details=host?.lastElementChild,item=details?.querySelector('ol li');if(!stage||!item)return;const note=document.createElement('p'),intentLabels={keep_after_review:'复核后保留方案',revise_after_review:'复核后修改方案'};note.className='status';note.textContent=stage.status==='not_reviewed'?'未生成或复核赛前多时点未来。':'分叉证据：'+(intentLabels[stage.review_intent]||'已复核')+' · '+Number(stage.eligible_scenarios||0)+' / '+Number(stage.fixed_scenario_budget||0)+' 个时点可用 · 动作分叉 '+Number(stage.action_divergence_scenarios||0)+' · 局部归因 '+Number(stage.local_attribution_scenarios||0)+' · 描述差异 '+Number(stage.descriptive_future_difference_scenarios||0)+' · 时间敏感性 '+(stage.timing_sensitivity_observed?'已观察到':'未观察到')+' · '+(stage.status==='selected_for_fixture'?'已进入最终选择':'未进入最终选择');item.append(note)};
const appendManagerWorldEvolutionThreadWithoutMechanismSemantics=appendManagerWorldEvolutionThread;
appendManagerWorldEvolutionThread=(host,thread)=>{appendManagerWorldEvolutionThreadWithoutMechanismSemantics(host,thread);const stage=(thread?.stages||[]).find(row=>row.stage_id==='prematch_future_review'),details=host?.lastElementChild,item=details?.querySelector('ol li');if(!stage||!item||stage.status==='not_reviewed')return;const note=document.createElement('p');note.className='status';note.textContent='动作语义：传中 '+Number(stage.cross_action_mechanism_examples||0)+' · 直接偏好 '+Number(stage.direct_preference_mechanism_examples||0)+' · 仅抑制 '+Number(stage.suppression_only_mechanism_examples||0)+' · 持球基准再分配 '+Number(stage.hold_reference_redistribution_examples||0)+' · 非零传中窗口 '+Number(stage.nonzero_cross_descriptive_windows||0);item.append(note)};
const appendManagerWorldEvolutionThreadWithoutOfficialActionSemantics=appendManagerWorldEvolutionThread;
appendManagerWorldEvolutionThread=(host,thread)=>{appendManagerWorldEvolutionThreadWithoutOfficialActionSemantics(host,thread);const stages=thread?.stages||[],index=stages.findIndex(row=>row.stage_id==='official_world_model_actions'),stage=stages[index],items=host?.lastElementChild?.querySelectorAll('ol li'),item=index>=0?items?.[index]:null;if(!stage||!item||!Number(stage.retained_semantic_examples||0))return;const note=document.createElement('p');note.className='status';note.textContent='正式动作有界样例：'+Number(stage.retained_semantic_examples||0)+' / '+Number(stage.retained_records||0)+' 条 · '+(stage.semantic_example_coverage_complete?'覆盖完整':stage.semantic_examples_truncated?'样例已截断':'覆盖不完整')+' · 传中 '+Number(stage.semantic_cross_action_examples||0)+' · 直接偏好 '+Number(stage.semantic_direct_preference_examples||0)+' · 仅抑制 '+Number(stage.semantic_suppression_only_examples||0)+' · 持球再分配 '+Number(stage.semantic_hold_reference_redistribution_examples||0)+' · 直接传中事件 '+Number(stage.semantic_direct_cross_ball_event_examples||0)+' · 不代表赛果因果';item.append(note)};
...
const appendManagerWorldEvolutionThreadWithoutScenarioArchive=appendManagerWorldEvolutionThread;
appendManagerWorldEvolutionThread=(host,thread)=>{appendManagerWorldEvolutionThreadWithoutScenarioArchive(host,thread);const stage=(thread?.stages||[]).find(row=>row.stage_id==='prematch_future_review'),item=host?.lastElementChild?.querySelector('ol li');appendReviewedScenarioArchive(item,stage?.reviewed_scenarios,'已封存的赛前分叉')} ;
...
const appendManagerWorldEvolutionThreadWithoutReviewWorldCertificate=appendManagerWorldEvolutionThread;
appendManagerWorldEvolutionThread=(host,thread)=>{appendManagerWorldEvolutionThreadWithoutReviewWorldCertificate(host,thread);const details=host?.lastElementChild;if(details?.tagName==='DETAILS')appendReviewWorldContinuity(details,thread?.review_to_official_world)};
...
const renderManagerWorldActionAdoptionLedgerWithoutWorldPropagation=renderManagerWorldActionAdoptionLedger;
...
const renderManagerWorldNavigatorWithoutActionTransitionDrilldown=renderManagerWorldNavigator;
renderManagerWorldNavigator=season=>{renderManagerWorldNavigatorWithoutActionTransitionDrilldown(season);renderManagerWorldActionTransitionDrilldown(season)};
...
renderSeason=(season,configured,history=[],historySummary={})=>{renderSeasonWithoutManagerFutureSets(season,configured,history,historySummary);renderManagerFutureSets(season,season?.next_manager_fixture);renderManagerInterventionWorkspace(season);renderManagerWorldNavigator(season)};

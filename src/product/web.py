    ...
    <div id="matchday-command-center" class="matchday-command" aria-labelledby="matchday-command-title" hidden>
      ...
      <section id="manager-world-navigator" aria-labelledby="manager-world-navigator-title" hidden>
        ...
        <section id="manager-world-story" class="world-story" aria-labelledby="manager-world-story-title" hidden>
          <h5 id="manager-world-story-title">&#26412;&#36718;&#20915;&#31574;&#19982;&#19990;&#30028;&#24433;&#21709;&#26723;&#26696;</h5>
          <p id="manager-world-story-summary" class="status" role="status" aria-live="polite" aria-atomic="true"></p>
          <ol id="manager-world-story-stages" class="world-story-rail" aria-label="&#26412;&#36718;&#20915;&#31574;&#19982;&#19990;&#30028;&#24433;&#21709;&#38142;"></ol>
          <div id="manager-world-story-dossier" class="cards" role="list" aria-label="&#21516;&#31456;&#20915;&#31574;&#24433;&#21709;&#26723;&#26696;"></div>
...
const managerWorldStory=document.querySelector('#manager-world-story'),managerWorldStorySummary=document.querySelector('#manager-world-story-summary'),managerWorldStoryStages=document.querySelector('#manager-world-story-stages'),managerWorldStoryDossier=document.querySelector('#manager-world-story-dossier'),managerWorldStoryOpen=document.querySelector('#manager-world-story-open');
...
function renderManagerSquadRules(season,managed){renderManagerRules(managed)}
const MANAGER_SQUAD_RENDER_STAGES=Object.freeze([renderManagerSquadBase,renderManagerSquadRules]);
function renderManagerSquad(season,managed){for(const renderStage of MANAGER_SQUAD_RENDER_STAGES)renderStage(season,managed)}
...
function renderMatchdayCommandIntelligence(season,configured){renderManagerIntelligence(season?.matchday_command_center,configured)}
function renderMatchdayCommandClubStrategy(season,configured){renderClubStrategyBriefing(season?.matchday_command_center?.club_strategy)}
const MATCHDAY_COMMAND_RENDER_STAGES=Object.freeze([renderMatchdayCommandBase,renderMatchdayCommandIntelligence,renderMatchdayCommandClubStrategy]);
function renderMatchdayCommand(season,configured){for(const renderStage of MATCHDAY_COMMAND_RENDER_STAGES)renderStage(season,configured)}
...
function renderManagerCareerRecruitment(season,history,historySummary,configured){configureRecruitmentWindow(season);for(const archived of history||[]){const transaction=archived?.recruitment_transaction;if(!transaction)continue;const names=(transaction.moves||[]).map(move=>`${move.incoming?.name||move.candidate_id} ← ${move.outgoing?.name||move.outgoing_player_id}`).join(' · '),node=card(`${archived.season_id} · 阵容建设`,`${names} · 使用 ${transaction.spent}/${transaction.budget} 点`);node.setAttribute('role','listitem');seasonHistory.append(node)}}
const MANAGER_CAREER_RENDER_STAGES=Object.freeze([renderManagerCareerBase,renderManagerCareerRecruitment]);
function renderManagerCareer(season,history,historySummary,configured){for(const renderStage of MANAGER_CAREER_RENDER_STAGES)renderStage(season,history,historySummary,configured)}
...
function submitClubSituation(context){const {form,path,payload}=context;if(form===managerDecisionForm&&path==='/api/v1/seasons/decision'&&currentClubSituation){payload.decision.club_event_choice={schema_version:1,event_id:currentClubSituation.event_id,event_identity:currentClubSituation.event_identity,choice_id:clubSituationChoice.value}}}
...
function renderProductExtensions(data){renderClubFinance(data.studio?.club_finance);renderSportingDirection(data.studio?.season);renderSportingReview(data.studio?.sporting_reviews);renderLeagueEcosystem(data.studio?.league_ecosystem);renderClubStrategies(data.studio?.season?.club_strategies);renderPlayerDevelopment(data.studio?.player_development);renderPlayerLifecycle(data.studio?.player_lifecycle);renderGlobalPlayerMarket(data.studio?.player_market);renderScouting(data.studio?.scouting);renderScoutingOutcomes(data.studio?.scouting_outcomes);}
function renderProductNavigation(data){renderUnifiedWorkflow(data);renderWorkspaceAreas(data)}
const PRODUCT_RENDER_STAGES=Object.freeze([renderBase,renderProductExtensions,renderProductNavigation]);
function render(data){for(const renderStage of PRODUCT_RENDER_STAGES)renderStage(data)}
...
function renderUnifiedWorkflowManagerJourney(data){renderManagerProductJourney(data.studio?.season,data.configured)}
const UNIFIED_WORKFLOW_RENDER_STAGES=Object.freeze([renderUnifiedWorkflowBase,renderUnifiedWorkflowArea,renderUnifiedWorkflowManagerJourney]);
function renderUnifiedWorkflow(data){for(const renderStage of UNIFIED_WORKFLOW_RENDER_STAGES)renderStage(data)}
...
function appendFutureSocietyDivergences(host,scenarios,title){if(!host||!Array.isArray(scenarios))return;const rows=scenarios.filter(scenario=>scenario?.society_divergence);if(!rows.length)return;const details=document.createElement('details'),heading=document.createElement('summary'),list=document.createElement('ol'),scopeLabels={tactical_controls:'战术控制',emotion_profile:'情绪',social_narrative_state:'社会叙事',psychological_state:'心理',referee_grievance:'裁判积怨'},fieldLabels={pressing_intensity:'压迫强度',risk_budget:'风险预算',line_height:'防线高度',rotation_aggressiveness:'轮换激进度',pride:'自豪',anger:'愤怒',shame:'羞耻',fear:'恐惧',determination:'决心',trust_index:'信任指数',polarization:'极化',narrative_fatigue:'叙事疲劳',morale:'士气',pressure:'压力',trust:'信任',risk_appetite:'风险偏好',conflict:'冲突',audience_activation:'观众激活',value:'数值'},countLabels={memory_records:'记忆记录',cognitive_memory_records:'认知记忆',beliefs:'信念',reflections:'反思'};heading.textContent=title+' · '+rows.length+' 个预注册时点';list.className='journal-list';list.setAttribute('aria-label','基线世界与干预世界的终局社会状态分叉');for(const scenario of rows){const divergence=scenario.society_divergence,item=document.createElement('li'),name=document.createElement('strong'),summary=document.createElement('p');name.textContent=Number(scenario.branch_minute||0).toFixed(1)+' 分钟分叉';summary.className='status';if(!divergence.available){summary.textContent='终局社会状态不可用：'+String(divergence.reason||'证据缺失')+'；不能解释为零变化。';item.append(name,summary);list.append(item);continue}summary.textContent='状态值差异 '+Number(divergence.state_change_count||0)+' · 记录差异 '+Number(divergence.count_delta_count||0)+' · 涉及球队 '+Number(divergence.teams_with_terminal_divergence||0);item.append(name,summary);const changes=document.createElement('ol');changes.setAttribute('aria-label',Number(scenario.branch_minute||0).toFixed(1)+'分钟终局社会状态差异');for(const row of divergence.count_deltas||[]){const change=document.createElement('li');change.textContent=row.team+' · '+(countLabels[row.field]||row.field)+'：Δ '+(Number(row.delta)>=0?'+':'')+Number(row.delta);changes.append(change)}for(const row of divergence.state_changes||[]){const change=document.createElement('li');change.textContent=row.team+' · '+(scopeLabels[row.scope]||row.scope)+' · '+(fieldLabels[row.field]||row.field)+'：'+Number(row.baseline).toFixed(3)+' → '+Number(row.treatment).toFixed(3);changes.append(change)}if(!changes.childElementCount){const change=document.createElement('li');change.textContent='已测终局社会状态完全一致。';changes.append(change)}item.append(changes);list.append(item)}const boundary=document.createElement('p');boundary.className='status';boundary.textContent='这是同一 seed 双世界的身份绑定终局差异；局部动作归因不会自动传递到社会状态、心理状态或赛果，也不代表现实球队。';details.append(heading,list,boundary);host.append(details)}
...
function renderManagerWorldModelAdviceStatusReset(){managerWorldModelAdviceSummary.className='status'}
const MANAGER_WORLD_MODEL_ADVICE_RENDER_STAGES=Object.freeze([renderManagerWorldModelAdviceStatusReset,renderManagerWorldModelAdviceBase]);
function renderManagerWorldModelAdvice(...args){for(const renderStage of MANAGER_WORLD_MODEL_ADVICE_RENDER_STAGES)renderStage(...args)}
...
function renderManagerFutureSetsSocietyDivergence(season,managed){const sets=season?.manager_future_sets||[],nodes=[...managerFutureSetList.children];for(const [index,item] of sets.entries())appendFutureSocietyDivergences(nodes[index],item.scenario_evidence||[],'查看复杂系统终局分叉')}
const MANAGER_FUTURE_SET_RENDER_STAGES=Object.freeze([renderManagerFutureSetsBase,renderManagerFutureSetsReviewActions,renderManagerFutureSetsScenarioEvidence,renderManagerFutureSetsMechanismExamples,renderManagerFutureSetsSocietyDivergence]);
...
function appendManagerWorldStoryPlayerChanges(host,world){const evidence=world?.player_changes;if(!evidence)return;if(!evidence.available){if(world.persistent_state_available){const gap=document.createElement('p');gap.className='status';gap.textContent='球员级持久变化不可用：'+String(evidence.reason||'证据缺失')+'；汇总计数不会被解释成球员明细。';host.append(gap)}return}const details=document.createElement('details'),heading=document.createElement('summary'),list=document.createElement('ol'),boundary=document.createElement('p'),changeLabels={updated:'状态更新',added:'加入世界状态',removed:'移出世界状态'},fieldLabels={matches_played:'出场',minutes_ema:'分钟负荷',goals:'进球',assists:'助攻',yellow_cards:'黄牌',red_cards:'红牌',suspension_matches_left:'停赛剩余',injury_matches_left:'伤停剩余',injury_severity:'伤情强度',medical_recovery_credit:'医疗恢复',form_ema:'竞技状态',media_sentiment:'媒体情绪',last_rating:'最近评分'};heading.textContent='查看球员级持久变化 · '+Number(evidence.visible_count||0)+' / '+Number(evidence.total_changed||0)+(evidence.players_truncated?'（已截断）':'');list.className='journal-list';list.setAttribute('aria-label','同章球员级持久世界变化');for(const row of evidence.players||[]){const item=document.createElement('li'),title=document.createElement('strong'),changes=document.createElement('p');title.textContent=String(row.name||row.player_id||'未知球员')+' · '+(changeLabels[row.change]||row.change);changes.className='status';const fields=(row.fields||[]).map(field=>{const name=String(field.field||''),label=name.startsWith('condition_delta.')?'状态·'+name.slice(16):(fieldLabels[name]||name);return label+' '+String(field.before)+' → '+String(field.after)});changes.textContent=(fields.join(' · ')||'球员进入或离开持久阵容状态')+(row.fields_truncated?' · 字段已截断':'');item.append(title,changes);list.append(item)}boundary.className='status';boundary.textContent='这是该场之后写入模拟器的球员状态事实；不证明经理选择或世界模型动作导致这些变化，也不是现实医疗判断。';details.append(heading,list,boundary);host.append(details)}
function renderManagerWorldStoryDossier(season){managerWorldStoryDossier.replaceChildren();const dossier=season?.manager_world_story?.chapter_dossier;if(!dossier?.available)return;const choice=dossier.manager_choice||{},review=dossier.future_review||{},action=dossier.official_action||{},transition=action.transition_evidence||{},world=dossier.world_after||{},metrics=world.metrics_delta||{},changes=world.transition_summary||{},society=world.society_transition||{},intentLabels={keep_after_review:'复核后保留原方案',revise_after_review:'复核后修改方案'},actionLabels={pending:'等待正式比赛',changed:'观察到局部动作改变',influenced_only:'概率受影响但采样未改变',no_influence:'未观察到非零影响',not_applicable:'稳定模式不适用',evidence_unavailable:'正式动作证据不可用'},actionNames={hold:'持球',pass:'传球',cross:'传中',shot:'射门',none:'无动作'},outcomeLabels={win:'胜',draw:'平',loss:'负'};const choiceText=choice.available?'选择 '+String(choice.selected_tactic)+' / '+String(choice.rotation)+(choice.runtime_verified?' · 运行时 '+String(choice.applied_tactic||'不可用')+(choice.runtime_matches_selection?' 已匹配':' 未匹配'):' · 等待运行时绑定'):'尚未冻结经理选择',reviewText=review.available?(intentLabels[review.intent]||'已复核')+' · 可用分叉 '+Number(review.eligible_scenarios||0)+' / '+Number(review.registered_scenarios||0)+' · 动作分叉 '+Number(review.action_divergence_scenarios||0)+' · 局部归因 '+Number(review.local_attribution_scenarios||0)+' · 时间敏感性 '+(review.timing_sensitivity_observed===true?'有':review.timing_sensitivity_observed===false?'无':'待观察'):(review.status==='pending'?'等待经理复核':'本轮未采用未来复核'),actionText=(actionLabels[action.status]||String(action.status||'证据不可用'))+' · 保留决策 '+Number(action.retained_records||0)+' · 非零影响 '+Number(action.influenced_decisions||0)+' · 局部动作改变 '+Number(action.locally_attributable_action_changes||0)+' · 直接球事件 '+Number(action.direct_ball_event_links||0),worldText=world.result_available?(outcomeLabels[world.outcome]||String(world.outcome||'结果'))+' · '+Number(world.points_earned||0)+' 分':'赛果待执行或证据不可用',choiceNode=card('1 · 经理选择',choiceText),reviewNode=card('2 · 反事实未来复核',reviewText),actionNode=card('3 · 正式动作采用',actionText),worldNode=card('4 · 同章世界后果',worldText);if(transition.available){const trace=document.createElement('p'),rows=(transition.transitions||[]).map(row=>(actionNames[row.baseline_action]||row.baseline_action)+'→'+(actionNames[row.actual_action]||row.actual_action)+' '+Number(row.count||0));trace.className='status';trace.textContent='实际动作转移：'+(rows.join(' / ')||'无')+(transition.expectation_available?' · 共享采样期望 '+Number(transition.expected_counterfactual_action_changes||0).toFixed(3):' · 精确期望不可用')+' · '+(transition.full_source_transition_distribution_authorized?'源转移覆盖完整':'源转移覆盖未授权')+' · 同章共现，不归因赛果';actionNode.append(trace)}else if(action.status!=='pending'){const gap=document.createElement('p');gap.className='status';gap.textContent='精确动作转移不可用：'+String(transition.reason||'证据缺失')+'；不能把缺失解释为零改变。';actionNode.append(gap)}if(world.persistent_state_available){const state=document.createElement('p');state.className='status';state.textContent='状态变化：疲劳 '+Number(metrics.team_fatigue_ema||0).toFixed(3)+' · 士气 '+Number(metrics.squad_morale_ema||0).toFixed(3)+' · 媒体压力 '+Number(metrics.team_media_pressure||0).toFixed(3)+' · 变化球员 '+Number(changes.changed_players||0)+' · 新伤 '+Number(changes.new_injuries||0)+' · 新停赛 '+Number(changes.new_suspensions||0)+(world.recovery_complete?' · 恢复阶段已记录':' · 恢复阶段待完成');worldNode.append(state)}appendManagerWorldStoryPlayerChanges(worldNode,world);if(society.available){const social=document.createElement('p');social.className='status';social.textContent='社会延续：记忆 +'+Number(society.memory_record_delta||0)+' · 认知记忆 +'+Number(society.cognitive_memory_delta||0)+' · 信念 +'+Number(society.belief_delta||0)+' · 反思 +'+Number(society.reflection_delta||0)+' · 状态域 '+(society.changed_state_fields||[]).join('/');worldNode.append(social)}for(const node of [choiceNode,reviewNode,actionNode,worldNode])node.setAttribute('role','listitem');managerWorldStoryDossier.append(choiceNode,reviewNode,actionNode,worldNode)}
...
function syncManagerWorldChapterTrajectorySelection(fixtureId){for(const button of managerWorldTrajectoryList.querySelectorAll('button[data-fixture-id]'))button.setAttribute('aria-pressed',String(button.dataset.fixtureId===fixtureId))}
const MANAGER_WORLD_CHAPTER_SELECTION_STAGES=Object.freeze([syncManagerWorldChapterSelectionBase,syncManagerWorldChapterTrajectorySelection]);
function syncManagerWorldChapterSelection(fixtureId){for(const syncStage of MANAGER_WORLD_CHAPTER_SELECTION_STAGES)syncStage(fixtureId)}
...
function appendManagerWorldStorySocietyChanges(host,society){if(!society?.available)return;const signed=value=>{const number=Number(value||0);return(number>=0?'+':'')+String(number)},summary=[...host.querySelectorAll('p.status')].find(node=>node.textContent.startsWith('社会延续：')),fields=(society.changed_state_fields||[]).join('/')||'无';if(summary)summary.textContent='社会延续：记忆 '+signed(society.memory_record_delta)+' · 认知记忆 '+signed(society.cognitive_memory_delta)+' · 信念 '+signed(society.belief_delta)+' · 反思 '+signed(society.reflection_delta)+' · 状态域 '+fields;const evidence=society.state_change_evidence;if(!evidence?.available){const gap=document.createElement('p');gap.className='status';gap.textContent='社会状态精确前后值不可用：'+String(evidence?.reason||'证据缺失')+'；不能把明细缺失解释为零变化。';host.append(gap);return}const details=document.createElement('details'),heading=document.createElement('summary'),list=document.createElement('ol'),boundary=document.createElement('p'),scopeLabels={tactical_controls:'战术控制',emotion_profile:'情绪',social_narrative_state:'社会叙事',psychological_state:'心理',referee_grievance:'裁判积怨'},fieldLabels={pressing_intensity:'压迫强度',risk_budget:'风险预算',line_height:'防线高度',rotation_aggressiveness:'轮换激进度',pride:'自豪',anger:'愤怒',shame:'羞耻',fear:'恐惧',determination:'决心',trust_index:'信任指数',polarization:'极化',narrative_fatigue:'叙事疲劳',morale:'士气',pressure:'压力',trust:'信任',risk_appetite:'风险偏好',conflict:'冲突',audience_activation:'观众激活',value:'数值'},valueText=value=>value===null?'无':Number(value).toFixed(3);heading.textContent='查看社会状态精确变化 · '+Number(evidence.visible_count||0)+' / '+Number(evidence.total_changes||0)+(evidence.changes_truncated?'（已截断）':'');list.className='journal-list';list.setAttribute('aria-label','同章社会与心理持久状态变化');for(const row of evidence.changes||[]){const item=document.createElement('li'),label=(scopeLabels[row.scope]||row.scope)+' · '+(fieldLabels[row.field]||row.field);item.textContent=label+'：'+valueText(row.before)+' → '+valueText(row.after);list.append(item)}boundary.className='status';boundary.textContent='这些是该场之后写入模拟器的社会、心理与战术控制状态；不证明经理选择、世界模型动作或赛果导致这些变化，也不代表现实球队。';details.append(heading,list,boundary);host.append(details)}
function renderManagerWorldStorySocietyChanges(season){const society=season?.manager_world_story?.chapter_dossier?.world_after?.society_transition,worldNode=managerWorldStoryDossier.lastElementChild;if(worldNode)appendManagerWorldStorySocietyChanges(worldNode,society)}
function renderManagerWorldReviewedSocietyDivergence(season){const points=season?.manager_world_navigator?.world_trajectory?.points||[],nodes=[...managerWorldTrajectoryList.children];for(const [index,point] of points.entries())appendFutureSocietyDivergences(nodes[index],point.reviewed_future_context?.scenarios||[],'该轮赛前复杂系统终局分叉')}
const MANAGER_WORLD_RENDER_STAGES=Object.freeze([renderManagerWorldNavigatorBase,renderManagerWorldStory,renderManagerWorldStoryDossier,renderManagerWorldStorySocietyChanges,renderManagerWorldActionAdoptionLedger,renderManagerWorldTrajectory,renderManagerWorldReviewedFutureContinuity,renderManagerWorldScenarioArchive,renderManagerWorldReviewedSocietyDivergence,renderManagerWorldReviewCertificate,renderManagerWorldReviewInfluence,renderManagerWorldRetainedRecordSemantics,renderManagerWorldTransitionPropagation,renderManagerWorldActionTransitionMap,renderManagerWorldExactActionExpectation,renderManagerWorldActionTransitionDrilldown]);

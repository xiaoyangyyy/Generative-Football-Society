...
function renderRelease(data){releaseGates.replaceChildren();const cp=data.control_plane||data.studio?.control_plane||{},release=cp.release,ex=cp.excellence||{};if(!release){releaseSummary.textContent='发布状态尚不可用。';return}const product=release.scores?.product??ex.tracks?.product?.score??'—',academic=release.scores?.academic??ex.tracks?.academic?.score??'—';releaseSummary.textContent=`产品 ${product}/100 · 学术 ${academic}/100 · 代码契约${release.code_ready?'通过':'未通过'} · 最终发布${release.release_ready?'就绪':'未就绪'} · ${release.open_gate_count} 个开放门禁`;for(const gate of release.gates||[]){const item=card(gate.label,gate.passed?'通过':'待完成');item.setAttribute('role','listitem');item.dataset.gateId=gate.id;releaseGates.append(item)}}

...
`data/evaluation/independent_action_reproduction_protocol_v2.json`. It is
registered but cannot start until the V2 action-policy external replication
decision and successful container-runtime/release evidence exist. The completed
60-run action-outcome decision is already bound as the primary result. No
independent reproduction has been performed. The earlier V1 handoff remains a
historical artifact and does not cover the repaired action-policy candidate.
...
exact 30-pair/60-run action-outcome protocol and then the fixed 72-run,
three-arm V2 action-policy replication protocol without training or provider
calls.
...
Any material deviation forces an `inconclusive` conclusion. Protocol, both
decisions, the already fixed variance-diagnosis branch, promotion or
nonpromotion outcome,
...
```bash
python scripts/verify_independent_reproduction.py \
  --review data/evaluation/independent_action_reproduction_v2/review.json
```

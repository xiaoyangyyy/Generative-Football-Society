...
def main():
    base=ROOT/"data/frame_world/v94_temporal_providers"; manifest=json.loads((base/"manifest.json").read_text()); expected={Path(x["file"]).resolve() for x in manifest["matches"] if Path(x["file"]).resolve().parent==base.resolve()}; actual={x.resolve() for x in base.glob("*.npz")}; providers=[]
    ...
    medians=[x["delay_quantiles_s"]["q50"] for x in providers]; gates={"three_providers":len(providers)==3,"three_metrica_matches":manifest["providers"]["metrica"]["matches"]>=3,"arrays_valid":all(x["valid"] for x in providers),"minimum_observed":all(x["observed_events"]>=1000 for x in providers),"observed_rate":all(x["observed_rate"]>=.8 for x in providers),"clock_median_range_s":max(medians)-min(medians)<=.5,"no_orphan_v92_arrays":actual==expected}; report={"version":"9.4.0-candidate","providers":providers,"gates":gates,"ready":all(gates.values())}; write_json_atomic(ROOT/"reports/acceptance/temporal_provider_quality_v94.json",report); print(json.dumps(report,indent=2)); return 0 if report["ready"] else 1

...
def fold(held,manifest,device,epochs=6):
    source=[x for x in manifest["matches"] if x["provider"]!=held]; target=[x for x in manifest["matches"] if x["provider"]==held]; train_items,dev_items=split_items(source); train=provider_sets(train_items,50000,9401); dev=provider_sets(dev_items,10000,9402); test=provider_sets(target,100000,9403); train_loader=balanced_loader(train,9404); dev_loader=combined_loader(dev); test_loader=combined_loader(test); torch.manual_seed(9400+("metrica","skillcorner","statsbomb360").index(held)); model=ContinuousTimeMarkModel(ContinuousTimeConfig()).to(device); cw=torch.tensor([label_weights(train,"continuation")],device=device); sw=torch.tensor([label_weights(train,"subtype")],device=device); opt=torch.optim.AdamW(model.parameters(),lr=7e-4,weight_decay=3e-3); best=None; history=[]

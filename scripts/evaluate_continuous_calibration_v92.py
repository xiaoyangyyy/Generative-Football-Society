...
def main():
    ...
    for held in ("metrica","skillcorner","statsbomb360"):
        source=[x for x in manifest["matches"] if x["provider"]!=held]; providers=sorted({x["provider"] for x in source}); model,_,meta=load_continuous_time(ROOT/f"data/frame_world/continuous_time_v92_lopo_{held}.pt"); model.eval(); source_radii={}; source_samples={}
        for provider in providers:
            data=Arrays(dev_items(source,provider),10000,9202); pred,_=predictions(model,data); observed=data.values["observed"]; truth=data.values["delay"][observed]; source_radii[provider]={str(level):radius(pred[observed],truth,level) for level in (.5,.9)}; source_samples[provider]=int(observed.sum())
        target=Arrays([x for x in manifest["matches"] if x["provider"]==held],100000,9203); pred,cdf=predictions(model,target); observed=target.values["observed"]; truth=target.values["delay"][observed]; intervals={str(level):interval(pred[observed],truth,max(source_radii[p][str(level)] for p in providers)) for level in (.5,.9)}; brier={}

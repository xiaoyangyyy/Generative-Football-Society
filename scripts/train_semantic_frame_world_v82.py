...
def train_fold(held, items, args, device):
    ...
    cfg = SemanticFrameConfig(pressure_enabled=pressure_enabled)
    sets = [
    ...
    torch.manual_seed(8200 + len(held))
    ...
    path = ROOT / f"data/frame_world/frame_world_v82_semantic_lodo_{held}.pt"
...
def main():
    ...
    report = {
        ...
        "policy": "strict provider LODO for pass/shot; pressure scored only where known",
        "folds": folds,
        "gates": {
            "strict_lodo": all(
    ...
    target = ROOT / "reports/acceptance/frame_world_v82.json"

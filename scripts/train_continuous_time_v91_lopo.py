...
def fold(held,manifest,device,epochs):
    ...
    model.load_state_dict(best[1]); train_anchor=float(np.median(train.values["delay"][train.values["observed"]])); model.time_anchor_s=train_anchor; model.time_shrinkage=.5; cut=thresholds(model,loaders[1],device); metrics=evaluate(model,loaders[2],device,*cut); tr=train.values["delay"][train.values["observed"]]; te=test.values["delay"][test.values["observed"]]; baseline=float(np.mean(np.abs(te-np.median(tr)))); artifact=ROOT/f"data/frame_world/continuous_time_v91_lopo_{held}.pt"; payload=model.checkpoint(); payload["meta"]={"held_provider":held,"train_providers":sorted({x["provider"] for x in source}),"continue_threshold":cut[0],"subtype_threshold":cut[1],"feature_contract":"v91_static_mirror_invariant_corrected_clock","time_anchor_s":train_anchor,"time_shrinkage":.5}; torch.save(payload,artifact); gates={"strict_provider_holdout":held not in {x["provider"] for x in source},"continuous_time_gain":metrics["time_mae_s"]<baseline,"continue_gain":metrics["continue_balanced_accuracy"]>.5,"mark_gain":metrics["pass_shot_balanced_accuracy"]>.5,"calibrated":metrics["event_2s_brier"]<.3,"finite":all(np.isfinite(x) for x in metrics.values())}; return {"held_provider":held,"train_providers":sorted({x["provider"] for x in source}),"samples":{"train":len(train),"dev":len(dev),"test":len(test)},"thresholds":{"continue":cut[0],"pass_shot":cut[1]},"metrics":metrics,"continuous_train_median_baseline_mae_s":baseline,"time_gain_s":baseline-metrics["time_mae_s"],"gates":gates,"ready":all(gates.values()),"artifact":artifact.relative_to(ROOT).as_posix(),"history":history}


def main():
    manifest=json.loads((ROOT/"data/frame_world/v91_temporal_providers/manifest.json").read_text()); device=torch.device("cuda" if torch.cuda.is_available() else "cpu"); folds=[fold(x,manifest,device,15) for x in ("skillcorner","statsbomb360")]; report={"version":"9.1.0-candidate","model":"three-component log-normal mixture survival","policy":"strict bidirectional provider LODO; continuous baseline gate","folds":folds,"ready":all(x["ready"] for x in folds)}; target=ROOT/"reports/acceptance/continuous_time_v91_lopo.json"; target.write_text(json.dumps(report,indent=2)+"\n"); print(json.dumps(report,indent=2)); return 0 if report["ready"] else 1

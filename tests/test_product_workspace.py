...
from src.product.club_strategy import (
    resolve_fixture_club_strategy, strategy_identity,
)
...
from src.product.decision_ledger import (
    build_manager_decision_ledger, validate_manager_decision_ledger,
)
from src.match_engine.tactical_catalog import TACTICAL_KEYS
from src.product.decision_advice import (
    build_manager_advice_adoption, build_manager_decision_advice,
)
...
from src.simulation.squad_registry import (
    RecruitmentMove, RecruitmentPlan, load_effective_roster,
)
...
class _Summary:
    ...
    world_model_action_adoption: dict = field(default_factory=lambda: {
        "available": True,
        "opportunities": 12,
...
def _runtime_tactical_execution(home, away, home_tactic, away_tactic):
    ...
    return {
        side: {
            "schema_version": 1, "team": team,
            "applied_tactic": tactic or "team_identity",
            "native_archetype": tactic or "balanced",
            "binding_kind": (
                "locked_preset" if tactic else "native_team_vector"
            ),
            "source": (
                "studio_user_intervention" if tactic else "agent_team_identity"
            ),
            "preset_locked": bool(tactic),
            "initial_vector": dict(vector),
            "final_vector": dict(vector),
            "changed_controls": [], "final_delta_l1": 0.0,
        }
...
def test_workflow_unifies_season_setup_decision_and_matchday_actions(tmp_path):
    ...
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Unified season", mode="research", seed=7),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"),
        manager_team="Brazil", manager_objective="top_half",
    ))
    ...
    assert setup["journey"][0] == {
        "id": "season_setup", "status": "action_required",
    }

    workspace.set_player_role_promises(PlayerPromisePlan((
        PlayerRolePromise("brazil-00", "core"),
    )))
    ...
    assert ready["journey"][2] == {
        "id": "matchday_execution", "status": "ready",
    }
    ...
    readiness_blocked["readiness"] = {
        "ready": False, "blockers": ["test_gate"],
    }
...
def test_workflow_prioritizes_active_spectator_season_over_single_match_loop(
    ...
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Spectator season", mode="stable", seed=8),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team=None,
    ))
...
def test_studio_match_writes_one_composed_product_report(tmp_path, monkeypatch):
    ...
    absolute_ball_log.write_text("\n".join([
        ...
        json.dumps({
            "type": "pass", "t_sec": 120, "team": "Brazil",
    ...
    assert report["layers"]["world_model"]["action_adoption"][
    ...
    assert report["replay"]["world_model_action_links"][
        "directly_observed"
    ] == 1
...
def test_match_history_is_bounded_latest_first_and_field_minimized(tmp_path):
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="History", mode="stable", seed=7),
    )
    session = json.loads(workspace.session_path.read_text(encoding="utf-8"))
    session["matches"] = [{
        "match_id": f"{index:04d}-home-vs-away",
        "home": "Home", "away": "Away", "seed": index, "fast": True,
        "score": {"home": index % 3, "away": 0},
        "integrity": "accepted",
        "match_plan": {
            "experience": "observational", "home_tactic": "team_identity",
...
def test_tactical_lab_uses_locked_presets_physics_score_and_explicit_seed(
    ...
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Lab", mode="research", seed=11),
    )
    observed = {}
    from src import app
    ...
    report = workspace.run_match(
        "Brazil", "Argentina", fast=True, plan=plan, seed_override=77,
    )
    ...
    second = workspace.run_match(
        "Brazil", "Argentina", fast=True, plan=second_plan,
    )
    ...
    treatment_dashboard = Path(second["dashboard_path"]).read_text(
        encoding="utf-8"
    )
    assert "打开同种子战术配对比较" in treatment_dashboard
    comparison = json.loads(
        Path(second["comparison_path"]).read_text(encoding="utf-8")
    )
    assert comparison["eligibility"]["eligible_for_tactical_attribution"]
    assert comparison["intervention"]["scope"] == (
    ...
    with pytest.raises(ValueError, match="same fast configuration"):
        workspace.run_match(
            "Brazil", "Argentina", fast=False, plan=second_plan,
        )
...
def test_workspace_paired_execution_enforces_single_side_physics_contract(
    ...
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Pair Guard", mode="research", seed=11),
    )
    unchanged_baseline = MatchPlan(
        experience="tactical_lab", home_tactic="balanced",
        away_tactic="low_block_counter",
    )
    unchanged_treatment = MatchPlan(
        experience="tactical_lab", home_tactic="balanced",
        away_tactic="low_block_counter", reuse_last_seed=True,
    )
    with pytest.raises(ValueError, match="exactly one changed"):
        workspace.run_paired_matches(
            ...
            treatment_plan=unchanged_treatment, seed=7,
        )
    joint_treatment = MatchPlan(
        experience="tactical_lab", home_tactic="gegenpress",
        away_tactic="direct_vertical", reuse_last_seed=True,
    )
    with pytest.raises(ValueError, match="exactly one changed"):
        workspace.run_paired_matches(
            "Brazil", "Argentina", fast=True,
            baseline_plan=unchanged_baseline,
            treatment_plan=joint_treatment, seed=7,
        )
    with pytest.raises(ValueError, match="tactical_lab physics"):
        workspace.run_paired_matches(
            "Brazil", "Argentina", fast=True,
            baseline_plan=MatchPlan(), treatment_plan=joint_treatment, seed=7,
        )
...
def test_paired_transaction_resumes_after_baseline_without_duplicate(
    ...
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Pair Resume", mode="research", seed=11),
    )
    ...
    with pytest.raises(RuntimeError, match="simulated treatment crash"):
        workspace.run_paired_matches(
            "Brazil", "Argentina", fast=True,
            baseline_plan=baseline_plan, treatment_plan=treatment_plan,
            seed=77, transaction_id=transaction_id,
        )
    session = json.loads(workspace.session_path.read_text(encoding="utf-8"))
    ...
    with pytest.raises(ValueError, match="identity conflict"):
        workspace.run_paired_matches(
            "Brazil", "Argentina", fast=True,
            baseline_plan=baseline_plan, treatment_plan=treatment_plan,
            seed=78, transaction_id=transaction_id,
        )
    assert len(calls) == 4
...
def test_stable_workspace_rejects_tactical_lab_before_reserving_run(tmp_path):
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Stable", mode="stable", seed=11),
    )
...
def test_paired_workspace_persists_synchronized_dual_replay(tmp_path, monkeypatch):
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Pair Replay", mode="research", seed=11),
    )
    from src import app
    ...
    def fake_match(home, away, **kwargs):
        ...
        path.write_text("\n".join([
            ...
            json.dumps({
                "type": "pass", "t_sec": t_sec, "team": home,
                "from": f"A{calls}", "to": f"B{calls}", "kind": "ground",
                "from_xy": [0.2, 0.3], "land_xy": [0.7, 0.4],
                "outcome": "COMPLETE",
            }),
        ]) + "\n", encoding="utf-8")
    ...
    _, treatment = workspace.run_paired_matches(
        "Brazil", "Argentina", fast=True,
        baseline_plan=baseline_plan, treatment_plan=treatment_plan,
        seed=77, transaction_id="pair-replay-001",
    )
    ...
    assert [
        frame["t_sec"] for frame in comparison["paired_replay"]["frames"]
    ] == [10, 20]
    assert comparison["paired_replay"]["event_correspondence_authorized"] is False
    dashboard = Path(treatment["comparison_dashboard_path"]).read_text(
        encoding="utf-8"
    )
...
def test_manager_decision_preview_is_zero_write_and_matches_submission(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Preview", mode="research", seed=11),
    )
    created = workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
    ))
    promised_id = created["manager_squad"]["players"][0]["player_id"]
    workspace.set_player_role_promises(PlayerPromisePlan((
        PlayerRolePromise(promised_id, "core"),
    )))
    fixture_id = workspace.season_status()["next_manager_fixture"]["fixture_id"]
    before = workspace.session_path.read_bytes()
    decision = ManagerDecision(
        team="Brazil", tactic="gegenpress", rotation="balanced",
    )
    ...
    assert preview["lineup"] == {
        "available": True, "source": "automatic", "starters": 11, "bench": 5,
    }
    assert preview["engine_tradeoff"] == {
        "status_penalty": 1.25, "rotation_level": 0.5,
    ...
    assert pending_ledger["entries"][0]["long_term_accounting"][
        "evidence_state"
...
def test_manager_execution_evidence_loading_is_bounded_to_recent_eight(
    ...
    season = {"fixtures": [
        {
            "fixture_id": f"fixture-{index:02d}", "state": "completed",
            "home": "Brazil", "away": f"Opponent {index}",
            "manager_decision": {}, "report": None,
        }
        for index in range(10)
    ]}

    evidence = workspace._manager_execution_evidence(
        season, manager_team="Brazil",
    )
...
def test_world_model_advice_is_persisted_previewed_and_explicitly_adopted(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Manager advisor", mode="research", seed=11),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
    ...
    fixture_id = workspace.season_status()["next_manager_fixture"]["fixture_id"]
    monkeypatch.setattr(
        workspace, "_generate_manager_decision_advice_packet",
    ...
    assert aligned_comparison["recommended_minus_selected"][
        "risk_adjusted_value"
    ] == 0.0
    submitted = workspace.set_manager_decision(
        decision, fixture_id=fixture_id,
    ...
    assert ledger_advice["adoption"]["adoption_identity"] == adopted[
        "adoption_identity"
    ]
    ...
    assert pending_trace["runtime_binding"] == {
        "status": "awaiting_execution", "reason": "fixture_not_completed",
    }
    ...
    assert reviewed_comparison["recommended_minus_selected"][
        "risk_adjusted_value"
    ] > 0.0
    ...
    manual_adoption = reviewed["next_manager_fixture"][
    ...
    protocol_evidence = workspace.status()["evidence"][
...
def test_world_model_advice_stale_link_and_stable_mode_fail_closed(
    ...
    stable = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Stable advisor boundary", mode="stable", seed=11),
    )
    stable.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
    ))
...
def test_adopted_world_model_advice_binds_to_direct_tactical_execution(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Advisor execution chain", mode="research", seed=11),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
    ...
    workspace.set_manager_decision(
        ...
        fixture_id=fixture_id, expected_revision=1,
        advice_adoption={
            "schema_version": 1,
            "advice_identity": advice["advice_identity"],
            "intent": "adopt_recommendation",
    ...
    def run(home, away, **kwargs):
        ...
        summary.manager_effects = {
            side: {
                "base_status": 70.0, "effective_status": 70.0,
                "rotation_status_penalty": 0.0,
                "fatigue_load_multiplier": 1.0,
                "club_fatigue_load_factor": kwargs[
                    f"{side}_fatigue_load_factor"
                ],
            }
            for side in ("home", "away")
        }
        summary.tactical_execution = _runtime_tactical_execution(
            home, away, kwargs["home_tactic"], kwargs["away_tactic"],
        )
        return summary
    ...
    assert trace["runtime_binding"]["binding_identity"] == binding[
        "binding_identity"
    ]
    ...
    tampered_entry["advisor_execution_trace"][
        "direct_recommendation_execution"
    ] = False
    entry_source = dict(tampered_entry)
    entry_source.pop("entry_identity")
    tampered_entry["entry_identity"] = hashlib.sha256(json.dumps(
        entry_source, sort_keys=True, separators=(",", ":"),
        ensure_ascii=False, allow_nan=False,
    ).encode("utf-8")).hexdigest()
    ledger_source = dict(tampered)
    ledger_source.pop("ledger_identity")
    tampered["ledger_identity"] = hashlib.sha256(json.dumps(
        ledger_source, sort_keys=True, separators=(",", ":"),
        ensure_ascii=False, allow_nan=False,
    ).encode("utf-8")).hexdigest()
    with pytest.raises(
        ValueError, match="manager advisor execution trace replay mismatch",
    ):
...
def test_world_model_advice_inference_releases_session_lease(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Advisor short lease", mode="research", seed=11),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
    ...
    monkeypatch.setattr(
        workspace, "_generate_manager_decision_advice_packet", generate,
    )
...
def test_world_model_advice_reuses_identical_current_inputs_without_inference(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Advisor retry reuse", mode="research", seed=11),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
...
def test_concurrent_identical_world_model_advice_requests_converge(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Advisor duplicate merge", mode="research", seed=11),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
    ...
    assert status["next_manager_fixture"]["manager_decision_advice"][
        "advice_identity"
    ] == results[0]["advice_identity"]
...
def test_world_state_change_recomputes_instead_of_reusing_advice(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Advisor reuse invalidation", mode="research", seed=11),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
...
def test_world_model_advice_does_not_overwrite_concurrent_manager_decision(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Advisor optimistic commit", mode="research", seed=11),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
    ...
    def generate(**kwargs):
        workspace.set_manager_decision(
            ManagerDecision(team="Brazil", tactic="gegenpress"),
            fixture_id=fixture_id, expected_revision=0,
        )
...
def test_world_model_advice_rejects_world_state_drift_without_writing(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Advisor state drift", mode="research", seed=11),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
...
def test_world_model_advice_rejects_checkpoint_drift_without_writing(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Advisor checkpoint drift", mode="research", seed=11),
    )
    workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
...
def test_workspace_season_completes_matchday_with_stable_continuity_ids(
    ...
    _manager_roster(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Season", mode="research", seed=11),
    )
    created = workspace.create_season(SeasonPlan(
        teams=("Brazil", "Argentina", "France", "Germany"), fast=True,
        manager_team="Brazil",
    ...
    assert created["matchday_command_center"]["primary_action"]["id"] == "submit_decision"
    ...
    monkeypatch.setattr(
        ...
        lambda base_dir, team_ids, **kwargs: recoveries.append({
            "base_dir": base_dir, "team_ids": list(team_ids), **kwargs,
        }),
    )
    decided = workspace.set_manager_decision(ManagerDecision(
        team="Brazil", tactic="gegenpress", rotation="strongest",
        in_match_plan=InMatchPlan("Brazil", (
            InMatchInstruction(
                "protect-lead-70", 70, "leading", tactic="low_block_counter",
            ),
        )),
    ))
    ...
    assert decided["next_manager_fixture"]["manager_decision"][
        "in_match_plan"
    ]["instructions"][0]["rule_id"] == "protect-lead-70"
    assert decided["next_manager_fixture"]["opponent_preparation"][
        "state"
    ...
    assert closed["long_term_accounting"]["evidence_state"] == (
        "completed_evidence"
    )
    ...
    assert ProductWorkspace.load(tmp_path).season_status()[
    ...
    managed = [call for call in observed if call["home_tactic"] == "gegenpress"
               or call["away_tactic"] == "gegenpress"]
    ...
    active_plan = (
        managed[0]["home_in_match_plan"]
        or managed[0]["away_in_match_plan"]
    )
    ...
    assert {call["continuity_id"] for call in observed} == {
        "season-0001:md01-fx01", "season-0001:md01-fx02",
    }
    ...
    second = workspace.set_manager_decision(ManagerDecision(
        team="Brazil", tactic="gegenpress", rotation="balanced",
    ))
    assert second["next_manager_fixture"]["opponent_preparation"][
        "evidence"
    ]["observation_count"] == 1
    workspace.play_next_matchday(expected_revision=2)
    third = workspace.set_manager_decision(ManagerDecision(
        team="Brazil", tactic="tiki_taka", rotation="balanced",
    ))
    ...
    applied = next(
        call for call in third_day_calls
        if call["home_tactic"] == "tiki_taka"
        or call["away_tactic"] == "tiki_taka"
    )
    ...
    assert recorded["match_plan"]["club_strategy"][
        "opponent_adaptation_applied"
    ] is True
    ...
    adapted_entry = next(
        entry for entry in archived["manager_profile"]["journal"]
    ...
    assert archived_world_entry["world_state_transition"][
        "transition_identity"
    ]
    ...
    advice_home = (
        ...
        if advice_entry["venue"] == "home" else advice_entry["opponent"]
    )
    advice_away = (
    ...
    archived_advice = build_manager_decision_advice(
        ...
        home=advice_home, away=advice_away,
        manager_team=advice_archive["plan"]["manager_team"],
        mode="research",
        match_seed=(
            advice_archive["seed"] + advice_entry["matchday"] * 100
            + advice_entry["order"]
        ),
        base_revision=0,
        checkpoint_artifact=(
    ...
    archived_adoption = build_manager_advice_adoption(
        archived_advice, selected_tactic=advice_entry["tactic"],
        intent=(
    ...
    assert workspace._season_history_view(advice_archive_session)[0][
        "manager_profile"
    ]["journal"][0]["manager_advice_adoption"] == archived_adoption
    tampered_advice_archive = json.loads(json.dumps(advice_archive_session))
    tampered_advice_archive["season_history"][0]["manager_profile"][
        "journal"
    ][0]["manager_advice_adoption"]["selected_rank"] = 99
    ...
    transition = next(
        ...
        for entry in tampered_world_session["season_history"][0][
            "manager_profile"
        ]["journal"]
        if entry.get("world_state_transition") is not None
    )
    manager_team = tampered_world_session["season_history"][0]["plan"][
        "manager_team"
    ]
    transition["match_delta"][manager_team]["metrics_delta"][
        "team_fatigue_ema"
    ] = 99.0
    transition_source = dict(transition)
    transition_source.pop("transition_identity")
    transition["transition_identity"] = hashlib.sha256(json.dumps(
        transition_source, sort_keys=True, separators=(",", ":"),
        ensure_ascii=False, allow_nan=False,
    ).encode("utf-8")).hexdigest()
    workspace.session_path.write_text(
        json.dumps(tampered_world_session), encoding="utf-8",
    )
    with pytest.raises(ValueError, match="world-state transition replay mismatch"):
        workspace.status()
    workspace.session_path.write_text(
        json.dumps(clean_archive_session), encoding="utf-8",
    )
    tampered = workspace._session()
    target_entry = next(
        entry for entry in tampered["season_history"][0]["manager_profile"]["journal"]
        if (entry.get("opponent_preparation") or {}).get("state") == "adapted"
    )
...
def test_workspace_manager_debrief_rejects_report_outside_workspace(tmp_path):
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Debrief Boundary", mode="research", seed=13),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), fast=True, manager_team="A",
    ))
    session = workspace._session()
    fixture = next(
        item for item in session["season"]["fixtures"]
        if "A" in {item["home"], item["away"]}
    )
    fixture.update({
        "state": "completed", "match_id": "escaped-report",
    ...
    debrief = workspace.season_status()["matchday_command_center"][
        "postmatch_debrief"
    ]
...
def test_workspace_archives_only_completed_season_before_starting_next(tmp_path):
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Career", mode="research", seed=23),
    )
    workspace.create_season(SeasonPlan(
    ...
    second = workspace.create_season(
        SeasonPlan(
            teams=("A", "B", "C", "D"), manager_team="A",
            manager_objective="points_target", manager_points_target=5,
        ),
        replace=True,
    )
    status = workspace.status()
    ...
    assert status["season_history_summary"] == {
        "total": 1, "retained": 1, "limit": 12, "truncated": False,
    }
...
def test_workspace_recruitment_atomically_changes_the_real_next_season_squad(
    ...
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Squad career", mode="research", seed=31),
    )
    ...
    outgoing = next(
        player for player in market["outgoing_players"]
        if player["role"] != "GK"
    )
    plan = RecruitmentPlan(market["market_id"], (
        RecruitmentMove(candidate["player_id"], outgoing["player_id"]),
    ))
    second = workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="top_half", manager_recruitment=plan,
    ), replace=True)
    ...
    player_ids = {
        player["player_id"] for player in second["manager_squad"]["players"]
    }
    assert candidate["player_id"] in player_ids
    assert outgoing["player_id"] not in player_ids
    signed = next(
        player for player in second["manager_squad"]["players"]
    ...
    assert [entry["type"] for entry in finance["ledger_entries"]] == [
        "season_settlement", "recruitment_charge",
    ]
    ...
    workspace.session_path.write_text(
        json.dumps(tampered_finance), encoding="utf-8",
    )
...
def test_workspace_develops_players_from_exact_minutes_and_replays_the_source(
    ...
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Development career", mode="research", seed=32),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="champion",
    ...
    assert evidence["player_totals"]["a-00"] == {
        "minutes": 270.0, "appearances": 3,
    }
    transaction = persisted["player_development"]["clubs"]["A"][
        "transactions"
    ][0]
    ...
    tampered["player_development"]["clubs"]["A"]["transactions"][0][
        "players"
    ][0]["season_delta"] = 0.0
    workspace.session_path.write_text(json.dumps(tampered), encoding="utf-8")
...
def test_workspace_retention_retirement_and_academy_change_the_real_squad(tmp_path):
    ...
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Lifecycle career", mode="research", seed=33),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="champion",
    ...
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="top_half", manager_retention=retention,
    ), replace=True)
    ...
    academy = [
        player for player in effective["players"]
    ...
    assert transaction["summary"] == {
        "renewed": 0, "continued": 14, "new_signings": 0,
        "contract_releases": 1, "retirements": 1, "academy_promotions": 2,
    }

    persisted = workspace._session()
    tampered = json.loads(json.dumps(persisted))
    tampered["season"]["plan"]["manager_retention"][
        "renew_player_ids"
    ] = ["a-01"]
    workspace.session_path.write_text(json.dumps(tampered), encoding="utf-8")
...
def test_global_market_preserves_player_identity_across_three_seasons(tmp_path):
    ...
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Global market", mode="research", seed=34),
    )
    ...
    def complete_active(manager_team):
        ...
        for fixture in session["season"]["fixtures"]:
            ...
            if manager_team in {fixture["home"], fixture["away"]}:
                fixture["manager_decision"] = {
                    "team": manager_team, "tactic": "balanced",
                    "rotation": "balanced",
                }
        session["season"]["state"] = "complete"
        workspace.session_path.write_text(json.dumps(session), encoding="utf-8")
    ...
    signing = FreeAgentPlan(
        market["market_id"], "a-01", outgoing["player_id"],
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="B",
        manager_objective="top_half", manager_free_agent=signing,
    ), replace=True)

    effective = load_effective_roster(tmp_path, "B")
    signed = next(player for player in effective["players"] if player["player_id"] == "a-01")
    ...
    tampered = json.loads(json.dumps(persisted))
    tampered["season"]["plan"]["manager_free_agent"][
        "outgoing_player_id"
    ] = "b-02"
    workspace.session_path.write_text(json.dumps(tampered), encoding="utf-8")
    ...
    tampered_outcome["scouting_outcomes"]["outcomes"][0]["realized"][
        "team_points"
    ] += 1
    workspace.session_path.write_text(
        json.dumps(tampered_outcome), encoding="utf-8",
    )
...
def test_sporting_director_plan_freezes_one_cross_module_next_season_policy(tmp_path):
    ...
    for team in ("A", "B", "C", "D"):
        _manager_roster(tmp_path, team)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Sporting director", mode="research", seed=41),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="top_half",
    ...
    outside_role = next(
        row["role"] for row in brief["role_diagnostics"]
        if row["role"] != candidate["role"]
    )
    outgoing = next(
        row for row in brief["outgoing_players"] if row["role"] != "GK"
    )
    conflicting_directive = SportingDirective(
        brief["planning_id"], philosophy="balanced", risk_level="high",
    ...
    directive = SportingDirective(
        brief["planning_id"], philosophy="balanced", risk_level="balanced",
    ...
    tampered["season"]["plan"]["manager_sporting_directive"][
        "planning_id"
    ] = rewritten_id
    tampered["season"]["sporting_evaluation"]["planning_id"] = rewritten_id
    tampered["season"]["sporting_evaluation"]["directive"][
        "planning_id"
    ] = rewritten_id
    workspace.session_path.write_text(json.dumps(tampered), encoding="utf-8")
...
def test_completed_sporting_plan_produces_replayable_cross_season_review(tmp_path):
    ...
    for team in ("A", "B", "C", "D"):
        _manager_roster(tmp_path, team)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Strategy review", mode="research", seed=43),
    )
    ...
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="top_half", manager_sporting_directive=directive,
    ), replace=True)
    complete_current_season()
    next_brief = workspace.sporting_plan("A")
    feedback = next_brief["previous_strategy_review"]
    assert feedback["season_id"] == "season-0002"
    assert feedback["priority_unaddressed_roles"] == sorted(
        directive.priority_roles
    )
    assert all(
    ...
    next_directive = SportingDirective(
        next_brief["planning_id"], philosophy="balanced",
        risk_level="balanced",
    ...
    row = next(
        item for item in review["post_squad"]
        if item["role"] not in priority
        and all(abs(item["quality"] - threshold) > 0.005 for threshold in (0.62, 0.70, 0.78))
    )
    ...
    workspace.session_path.write_text(json.dumps(tampered), encoding="utf-8")
    with pytest.raises(
        ValueError, match="sporting (brief|review) source replay mismatch",
    ):
        workspace.status()
...
def test_club_situation_is_an_atomic_manager_choice_with_source_replay(tmp_path):
    ...
    for team in ("A", "B", "C", "D"):
        _manager_roster(tmp_path, team)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Club timeline", mode="research", seed=47),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="top_half",
    ...
    session = workspace._session()
    managed = [
        row for row in session["season"]["fixtures"]
    ...
    for fixture in managed[:2]:
        fixture.update({
            ...
            "manager_decision": ManagerDecision(
                "A", tactic="balanced", rotation="strongest",
            ).as_dict(),
        })
    workspace.session_path.write_text(json.dumps(session), encoding="utf-8")
    ...
    protect = ClubEventChoice(
        situation["event_id"], situation["event_identity"], "protect_squad",
    )
    before = workspace.session_path.read_bytes()
    with pytest.raises(ValueError, match="conflicts with the club situation"):
        workspace.set_manager_decision(ManagerDecision(
            "A", tactic="balanced", rotation="strongest",
    ...
    push = ClubEventChoice(
        situation["event_id"], situation["event_identity"], "push_starters",
    )
    updated = workspace.set_manager_decision(ManagerDecision(
        "A", tactic="balanced", rotation="strongest",
    ...
    resolution["resolution_id"] = hashlib.sha256(json.dumps(
        resolution, sort_keys=True, ensure_ascii=False, separators=(",", ":"),
    ).encode("utf-8")).hexdigest()
    workspace.session_path.write_text(json.dumps(tampered), encoding="utf-8")
...
def test_workspace_finance_blocks_unaffordable_recruitment_atomically(tmp_path):
    ...
    for player in expensive["players"]:
        player["abilities"] = {
            key: 0.82 for key in player["abilities"]
        }
    roster_path.write_text(json.dumps(expensive), encoding="utf-8")
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Finance gate", mode="research", seed=37),
    )
    active = workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="champion",
    ))
    ...
    recruitment = RecruitmentPlan(market["market_id"], (
        RecruitmentMove(candidate["player_id"], outgoing["player_id"]),
    ))
    before = workspace.session_path.read_bytes()
    with pytest.raises(ValueError, match=r"available club funds \(3\)"):
        workspace.create_season(SeasonPlan(
            teams=("A", "B", "C", "D"), manager_team="A",
            manager_objective="top_half", manager_recruitment=recruitment,
        ), replace=True)
    assert workspace.session_path.read_bytes() == before
...
def test_workspace_replays_ai_club_evolution_into_next_season_rosters(tmp_path):
    ...
    for team in ("A", "B", "C", "D"):
        _manager_roster(tmp_path, team)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Living league", mode="research", seed=43),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="champion",
    ...
    tampered_strategy["season"]["club_strategies"]["profiles"][0][
        "style_scores"
    ]["balanced"] += 0.01
    workspace.session_path.write_text(
        json.dumps(tampered_strategy), encoding="utf-8",
    )
    ...
    tampered = workspace._session()
    tampered["league_ecosystem"]["transitions"][0]["ai_clubs"][0][
        "decision"
...
def test_workspace_enforces_board_contract_atomically_before_next_season(tmp_path):
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Board contract", mode="research", seed=41),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
        manager_objective="top_half",
    ...
    tampered = workspace._session()
    tampered["season_history"][0]["manager_profile"]["commitments"][
        "entries"
    ][1]["status"] = "missed"
    workspace.session_path.write_text(json.dumps(tampered), encoding="utf-8")
...
def test_named_player_promises_freeze_before_decisions_and_replay_roster_source(
    ...
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Player promises", mode="research", seed=47),
    )
    created = workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
    ))
    ...
    with pytest.raises(ValueError, match="age 23 or younger"):
        workspace.set_player_role_promises(PlayerPromisePlan((
            PlayerRolePromise("a-15", "development"),
        )))
    assert workspace.session_path.read_bytes() == original

    promised = workspace.set_player_role_promises(PlayerPromisePlan((
    ...
    assert [(row["player_id"], row["promised_role"]) for row in contract["players"]] == [
        ("a-00", "core"), ("a-01", "development"),
    ]
    frozen = workspace.session_path.read_bytes()
    with pytest.raises(ValueError, match="already frozen"):
        workspace.set_player_role_promises(PlayerPromisePlan((
            PlayerRolePromise("a-02", "rotation"),
        )))
    ...
    contract["contract_id"] = hashlib.sha256(json.dumps(
        identity_source, sort_keys=True, separators=(",", ":"),
        ensure_ascii=False, allow_nan=False,
    ).encode("utf-8")).hexdigest()
    workspace.session_path.write_text(json.dumps(tampered), encoding="utf-8")
...
def test_player_promise_archive_joins_lineup_accountability_to_exact_minutes(
    ...
    _manager_roster(tmp_path, "A")
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Promise outcomes", mode="research", seed=53),
    )
    created = workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), manager_team="A",
    ))
    ...
    assert workspace._season_history_view(replay_session)[0][
        "player_promise_outcomes"
    ] == outcome
...
def test_workspace_requires_next_manager_decision_before_any_fixture_starts(
    ...
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Decision gate", mode="research", seed=17),
    )
    status = workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), fast=True, manager_team="A",
    ))
    from src import app
    ...
    calls = []
    monkeypatch.setattr(
        app, "run_micro_match", lambda *args, **kwargs: calls.append(kwargs) or _Summary(),
    )
    with pytest.raises(ValueError, match="manager decision is required"):
        workspace.play_next_matchday()
    assert calls == []
    assert all(fixture.get("attempts", 0) == 0 for fixture in workspace.season_status()["fixtures"])

    next_fixture = status["next_manager_fixture"]
    later_fixture = next(
        fixture for fixture in status["fixtures"]
...
def test_workspace_rejects_stale_manager_decision_revision(tmp_path):
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Revision gate", mode="research", seed=19),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), fast=True, manager_team="A",
    ))
...
def test_workspace_season_retries_failed_fixture_with_same_settlement_identity(
    ...
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Retry Season", mode="research", seed=3),
    )
    workspace.create_season(SeasonPlan(
        teams=("A", "B", "C", "D"), fast=True, manager_team="A",
    ))
    ...
    before_identity = failed_fixture["world_state_before"][
        failed_fixture["home"]
    ]["source_identity"]
    ...
    assert retried_transition["phases"]["before_match"][
        status["fixtures"][0]["home"]
...
def test_workspace_season_adopts_existing_completed_report_without_rerun(
    ...
    _evidence(tmp_path)
    workspace = ProductWorkspace.create(
        tmp_path, StudioConfig(name="Recover Season", mode="research", seed=5),
    )
    ...
    workspace._run_match_locked(
        fixture["home"], fixture["away"], fast=True,
        plan=MatchPlan(
            experience="season_manager",
            home_tactic=club_strategy["home_tactic"],
            away_tactic=club_strategy["away_tactic"],
        ),
        seed_override=106, continuity=True,
        continuity_id="season-0001:md01-fx01", competition=competition,
